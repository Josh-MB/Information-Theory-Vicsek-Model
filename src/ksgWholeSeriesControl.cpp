...
)
{
	...
	if(sp.sims[MIBIN])
	{
		...
		for(size_t u = 0; u < recordedU; ++u) {
			...
			for(size_t i = 0; i < elements; i++) {
				dataForMI.setData(awrap(h[u + recordedU * interactions[u][i * 2]] - cumulativeRotations[u]), j, 0);
				dataForMI.setData(awrap(h[u + recordedU * interactions[u][i * 2 + 1]] - cumulativeRotations[u]), j, 1);
				j++;
			}
		}
	...
	if(sp.sims[GTEBIN])
	{
		ScopeTimer st("GTE (whole)");

		fmt::print("Calculating GTE\n");
		size_t U1 = recordedU - 1; //Reduced by one for tebin and gtebin, for past of X. 
		double gte = 0.0;
		if(sp.ksg_gte_dims == 2) {
			DataSet<2> dataForGTE((int)(gp.N * (recordedU - 1)), (int)sp.KSG_neighbours);
			int j = 0;
			for(size_t u = 0; u < U1; ++u) {
				for(size_t i = 0; i < gp.N; ++i, ++j) {
					dataForGTE.setData(awrap(h[u + i * recordedU] - cumulativeRotations[u]), j, 0);
					dataForGTE.setData(awrap(h[u+1+i * recordedU] - cumulativeRotations[u+1]), j, 1);
				}
			}
			gte = calculateReducedGTE(dataForGTE, gp.eta);
		}
		else if(sp.ksg_gte_dims == 3) {
			...
			for(size_t u = 0; u < U1; ++u) {
				for(size_t i = 0; i < gp.N; i++, ++j) {
					int t = (int)(u);		//Current timestep (i.e. for Xt and Yt)
					int t1 = (int)(u + offset);			//Next timestep (i.e. for Xt+1)

					dataForGTE.setData(awrap(h[t + i * recordedU] - cumulativeRotations[t]), j, 0);
					//When rotation is on, hAgg is calculated from the post rotated state, so we need
					//to use the accumulated rotations from the next time step (such that hAgg[t] is in the same
					//frame of reference as hFull[t] and hFull[t+1]
					dataForGTE.setData(awrap(hAggFull[t + i * recordedU] - cumulativeRotations[t1]), j, 1);
					dataForGTE.setData(awrap(hnew[t1 + i * recordedU] - cumulativeRotations[t1]), j, 2);
				}
			}
			...
			PEEXIT("KSG not supported for ksg_gte_dims={}. Please use either 0, 2 or 3", sp.ksg_gte_dims);
		}
