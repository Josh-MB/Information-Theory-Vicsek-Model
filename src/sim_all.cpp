...
int sim_all(int argc, char* argv[])
{
	...
	double* const hAgg_history = (double* const)calloc(gp.N*recordedU, sizeof(double));
	...
	*********************/
	...
	for(size_t u = 0; u < sp.U; ++u) { // for each update
		...
		connectedSet.clear();
		if(gp.imethod == 0) {
			vicsekMetric(gp.N, gp.L, gp.v, gp.eta, fs, hAgg, rng, gp.umethod, vicsekTree, interactionBuffer, 0, gp.discretise, &connectedSet, gp.rotate_frame, &rotations[buffer_u]);
		}
		else if(gp.imethod == 1) {
			vicsekTopological(gp.N, gp.L, gp.v, gp.eta, dt, fs, hAgg, rng, gp.topo_neighbours, vicsekTree, gp.umethod, interactionBuffer);
		}
			...
			inertialSpinModel(gp.N, gp.L, gp.v, sigma, gp.chi, gp.viscosity, gp.J, dt, gp.topo_neighbours,
							  fs, s, snew, hAgg,
							  engine, gaussian_rng, gp.umethod, vicsekTree, hamiltonian, interactionBuffer, 0, 0);
		}
	...
	if(sp.ksg_w)
		whole_results = fillAndRunWholeSeriesKSG(gp, sp, allInteractions, entire_history, hAgg_history, cumulativeRotations, fullgtennIdx, posPts, rng);
